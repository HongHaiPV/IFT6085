...
class SoftmaxLoss(nn.Module):
    ...
    def forward(self, feat, target):
        logits = nn.Linear(self.feat_dim, self.num_classes)(feat)
...
class LMCL(nn.Module):
    ...
    def forward(self, feat, target):
        ...
        y_onehot.scatter_(1, torch.unsqueeze(target, dim=-1), self.m)
        margin_logits = self.s * (logits - y_onehot)

        loss = nn.CrossEntropyLoss()(margin_logits, target)
        _, y_pred = torch.max(logits.data, 1)
        return loss, y_pred
...
class LGMLoss(nn.Module):
    """
        ...
        2018 07/02
    """
    def __init__(self, num_classes, loss_weight,  feat_dim, alpha):
    ...
    def forward(self, feat, target):
        ...
        y_onehot.zero_()
        y_onehot = Variable(y_onehot).cuda()
        y_onehot.scatter_(1, torch.unsqueeze(target, dim=-1), self.alpha)
        ...
        cdiff = feat - torch.index_select(self.centers, dim=0, index=target.long())
        cdist = cdiff.pow(2).sum(1).sum(0) / 2.0

        slog_covs = torch.squeeze(slog_covs)
        reg = 0.5*torch.sum(torch.index_select(slog_covs, dim=0, index=target.long()))
        likelihood = (1.0/batch_size) * (cdist + reg)

        loss = nn.CrossEntropyLoss()(margin_logits, target) + self.loss_weight * likelihood
        _, y_pred =  torch.max(logits.data, 1)
        return loss, y_pred
...
class CenterLoss(nn.Module):
    def __init__(self, num_classes, loss_weight, feat_dim):
    ...
    def forward(self, target, feat):
        ...
        logits = nn.Linear(self.feat_dim, self.num_classes)(feat)
        loss = nn.CrossEntropyLoss()(logits, target) + self.centerlossfunction(feat, target, self.centers) * self.loss_weight
        _, y_pred = torch.max(logits.data, 1)
